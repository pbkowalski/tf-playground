...
num_hidden_2 = 1024 # 1st layer num features
...
#    iterator = dataset.make_one_shot_iterator()
 #   imgs_it = iterator.get_next()
    canvas_orig = np.empty((85 * n, 68 * n))
    canvas_recon = np.empty((85 * n, 68 * n))
    for i in range(n):
        imgs = sess.run(imgs_it)
        imgs = np.reshape(imgs, (imgs.shape[0], num_input))
        ...
        for j in range(n):
            ...
            canvas_orig[i * 85:(i + 1) * 85, j * 68:(j + 1) * 68] = \
                imgs[j].reshape([85, 68])
        # Display reconstructed images
        for j in range(n):
            # Draw the reconstructed digits
            canvas_recon[i * 85:(i + 1) * 85, j * 68:(j + 1) * 68] = \
                g[j].reshape([85, 68])
